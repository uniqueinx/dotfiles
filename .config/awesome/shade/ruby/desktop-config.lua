...
function desktop:init(args)
	...
	netspeed.args = {
		...
		interface      = "wlp0s20f3",
		maxspeed       = { up = 3*1024^2, down = 3*1024^2 },
		crit           = { up = 3*1024^2, down = 3*1024^2 },
		timeout        = 2,
		autoscale      = true,
		label          = "NET"
	}
	...
	local ssd_smart_check = system.simple_async("smartctl --attributes /dev/nvme0n1p1", "Temperature:%s+(%d+)%sCelsius")
