...
function hotkeys:init(args)
	...
	self.raw.client = {
		{
			{ env.mod, "Shift" }, "f", function(c) c.fullscreen = not c.fullscreen; c:raise() end,
	...
	for i = 1, tcn do
		self.keys.root = awful.util.table.join(
			...
			tag_numkey(i, { env.mod, "Shift" }, function() client_move_and_go_by_index(i, { client.focus }) end)
		)
	end
	...
	for i, k in ipairs(tagkeys) do
		self.keys.root = awful.util.table.join(
			...
			awful.key({ env.mod, "Shift" }, k, function() client_move_and_go_by_index(i + tcn, { client.focus })    end)
		)
	end
